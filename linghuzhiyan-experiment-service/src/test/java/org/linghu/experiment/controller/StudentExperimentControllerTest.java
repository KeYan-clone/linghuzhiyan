...
import org.linghu.experiment.dto.*;
import org.linghu.experiment.service.StudentExperimentService;
...
class StudentExperimentControllerTest {
    ...
    void setUp() {
        ...
        // 创建测试任务DTO
        sampleTaskDTO = new ExperimentTaskDTO();
        sampleTaskDTO.setId("task123");
        sampleTaskDTO.setTitle("测试任务标题");
        sampleTaskDTO.setDescription("测试任务描述");
        sampleTaskDTO.setExperimentId("exp123");
        sampleTaskDTO.setOrderNum(1);
        sampleTaskDTO.setTaskType(org.linghu.experiment.constants.TaskType.CODE);
        sampleTaskDTO.setRequired(true);

        // 创建测试提交DTO
        sampleSubmissionDTO = new ExperimentSubmissionDTO();
        sampleSubmissionDTO.setId("sub123");
        sampleSubmissionDTO.setTask_id("task123");
        sampleSubmissionDTO.setUser_id("student123");
        sampleSubmissionDTO.setUser_answer("测试提交内容");
        sampleSubmissionDTO.setSubmission_time(LocalDateTime.now());

        // 创建测试评估DTO
        sampleEvaluationDTO = new ExperimentEvaluationDTO();
        sampleEvaluationDTO.setId("eval123");
        sampleEvaluationDTO.setSubmissionId("sub123");
        sampleEvaluationDTO.setTaskId("task123");
        sampleEvaluationDTO.setUserId("student123");
        sampleEvaluationDTO.setScore(new java.math.BigDecimal("85"));
        sampleEvaluationDTO.setUserAnswer("良好的解决方案");
        sampleEvaluationDTO.setSubmitTime(LocalDateTime.now());

        // 创建提交请求DTO
        submissionRequestDTO = new SubmissionRequestDTO();
        submissionRequestDTO.setTaskId("task123");
        submissionRequestDTO.setUserAnswer("测试提交内容");
