...
import org.linghu.user.config.SecurityConfig;
import org.linghu.user.config.TestJwtConfig;
...
class UserControllerTest {
    ...
    void getUserRoles_Success() throws Exception {
        Set<String> roles = Set.of("ROLE_STUDENT", "ROLE_USER");
        when(userService.getUserRoleIds("u123")).thenReturn(roles);

        mockMvc.perform(get("/api/users/u123/roles"))
                .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser(roles = "STUDENT")
    void getUserRoles_Failure_Forbidden() throws Exception {
        mockMvc.perform(get("/api/users/u123/roles"))
                .andExpect(status().isForbidden());
    }
}
